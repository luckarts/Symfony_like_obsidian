...
class OAuth2UserResolveListenerTest extends TestCase
{
    ...
    public function unknown_user_logs_and_persists_user_not_found_without_leaking_to_response(): void
    {
        ...
        $this->securityLogger
            ->expects($this->once())
            ->method('warning')
            ->with('login_failed', $this->callback(fn (array $context) => 'user_not_found' === $context['reason'] && null === $context['userId']));

        $this->securityEventRepository
            ->expects($this->once())
            ->method('save')
            ->with($this->callback(function (SecurityEvent $event) {
    ...
    public function wrong_password_logs_and_persists_bad_password_with_user_reference(): void
    {
        ...
        $this->securityLogger
            ->expects($this->once())
            ->method('warning')
            ->with('login_failed', $this->callback(fn (array $context) => 'bad_password' === $context['reason'] && $user->getId() === $context['userId']));
    ...
    public function valid_credentials_log_and_persist_login_success_and_set_user(): void
    {
        ...
        $this->securityLogger
            ->expects($this->once())
            ->method('info')
            ->with('login_success', $this->callback(fn (array $context) => $user->getId() === $context['userId']));
