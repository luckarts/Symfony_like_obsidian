...
class OAuth2UserResolveListener
{
    ...
    public function __invoke(UserResolveEvent $event): void
    {
        ...
        if (!$this->passwordHasher->isPasswordValid($securityUser, $event->getPassword())) {
            ...
            $this->securityEventRepository->save(
                SecurityEvent::loginFailed('bad_password', $event->getUsername(), $ip, $userAgent, $securityUser->getUser()->getId()),
            );
        ...
        $this->securityEventRepository->save(
            SecurityEvent::loginSuccess((string) $securityUser->getUser()->getId(), $event->getUsername(), $ip, $userAgent),
        );
